...
if (botSpam.antispam && m.text && user && user.lastCommandTime && (Date.now() - user.lastCommandTime) < 5000 && !isROwner) {
  ...
  user.commandCount = 1
        ...
        if (!m.fromMem && m.text.match(/(@1 (646) 461-7021|بوت|ميكو|MIKU| bot|Miku)/gi)) {
        let emot = pickRandom(["🎃", "❤", "😘", "😍", "💕", "😎", "🙌", "⭐", "👻", "🔥"])
...
export async function callUpdate(callUpdate) {
    ...
    //await this.sendContact(nk.from, data.map(([id, name]) => [id, name]), false, { quoted: callmsg })
     let vcard = `BEGIN:VCARD\nVERSION:3.0\nN:;mg3.g 👑;;;\nFN:mg3.g 👑\nORG:mg3.g 👑\nTITLE:\nitem1.TEL;waid=968 9904 4902:+968 9904 4902 \nitem1.X-ABLabel:mg3.g👑\nX-WA-BIZ-◤Shadow - Bot ◢:[❗] كلم الرقم ذا - المطور.\nX-WA-BIZ-NAME:mg3.g 👑\nEND:VCARD`
    await this.sendMessage(nk.from, { contacts: { displayName: 'mg3.g👑', contacts: [{ vcard }] }}, {quoted: callmsg})
...
global.dfail = (type, m, conn) => {
    let msg = {
        ...
        group: '*『 الميزه في الجروبات فقط ! 』*',
        private: '*『 الميزه للبرايفت - الخاص فقط ! 』*',
        admin: '*『 الميزه للمشرفين فقط! 』*',
        botAdmin: '*『 ارفع البوت ادمن الاول ! 』*',
        unreg: '*[ لحظة !! انت مش مسجل ]*\n\n*『 سجل الامر عشان تفعله 』*\n*➣ #تفعيل*',
        restrict: '*『 الميزه  المالك حذفها ! 』*'
    }[type]
    let aa = { quoted: m, userJid: conn.user.jid }
    let prep = generateWAMessageFromContent(m.chat, { extendedTextMessage: { text: msg, contextInfo: { externalAdReply: { title: '[ ⚠ ] تنبيه - إشعار', body: 'MIKU - ʙᴏᴛ', thumbnail: imagen1, sourceUrl: '--' }}}}, aa)
